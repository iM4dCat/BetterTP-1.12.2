...
public class CommandRotationShake extends CommandBase {
    ...
    public void execute(MinecraftServer p_184881_1_, ICommandSender p_184881_2_, String[] p_184881_3_) throws CommandException {
        if (p_184881_3_.length != 3) {
            throw new WrongUsageException("/rotationShake <entity> <pitch> <yaw>");
        } else {
            Entity lvt_5_2_;
            ...
            if (lvt_5_2_.world != null) {
                ...
                teleportEntityToCoordinates(lvt_5_2_, shakePitch * rd.nextDouble() * (rd.nextBoolean() ? -1 : 1), shakeYaw * rd.nextDouble()* (rd.nextBoolean() ? -1 : 1));
                notifyCommandListener(p_184881_2_, this, "successful", lvt_5_2_.getName());
            }
        }
    }

    private static void teleportEntityToCoordinates(Entity p_189863_0_, double shakePitch, double shakeYaw) {
        float lvt_7_1_;
